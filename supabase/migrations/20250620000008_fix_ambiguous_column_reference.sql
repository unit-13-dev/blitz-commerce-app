-- Fix ambiguous column reference error in create_individual_participant_payment function
-- The issue is with the UPDATE statement that doesn't specify the table name clearly

-- Update the create_individual_participant_payment function to fix column ambiguity
...
BEGIN
  ...
  -- Update group order status with explicit table references
  ...
  SET 
    ...
    total_amount = (SELECT COALESCE(total_amount, 0) + final_price FROM public.group_order_status WHERE group_id = group_uuid),
    updated_at = NOW()
