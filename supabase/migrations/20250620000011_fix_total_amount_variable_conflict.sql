-- Fix total_amount variable conflict in update_group_order_status function
-- The variable 'total_amount' conflicts with the column name 'total_amount'

-- Update the update_group_order_status function to fix total_amount variable conflict
...
DECLARE
  paid_count INTEGER;
  calculated_total_amount DECIMAL(10,2);  -- Renamed variable to avoid conflict
...
BEGIN
  ...
  INTO paid_count, calculated_total_amount, calculated_total_quantity  -- Use renamed variables
  FROM public.group_order_payments
  WHERE group_id = group_uuid AND payment_status = 'paid';
  
  RAISE NOTICE 'Paid participants: %, Total amount: %, Total quantity: %', paid_count, calculated_total_amount, calculated_total_quantity;
  ...
  IF FOUND THEN
    ...
    UPDATE public.group_order_status SET
      ...
      total_amount = calculated_total_amount,  -- Use renamed variable
      updated_at = NOW()
  ...
  ELSE
    ...
    INSERT INTO public.group_order_status (
      ...
      calculated_total_amount,  -- Use renamed variable
      'active',
      NOW(),
      NOW()
    );
