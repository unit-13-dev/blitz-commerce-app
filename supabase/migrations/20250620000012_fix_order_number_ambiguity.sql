-- Fix order_number ambiguity in create_individual_participant_payment function
-- The variable 'order_number' conflicts with the column name 'order_number' in the orders table

-- Update the create_individual_participant_payment function to fix order_number ambiguity
...
DECLARE
  ...
  current_total_amount DECIMAL;  -- Renamed variable to avoid conflict
BEGIN
  -- Get group details
  ...
  -- Get current total amount to avoid ambiguity
  SELECT COALESCE(total_amount, 0) INTO current_total_amount 
  FROM public.group_order_status 
  WHERE group_id = group_uuid;

  -- Update group order status with explicit variable names
  UPDATE public.group_order_status 
  SET 
    paid_participants = (SELECT COALESCE(paid_participants, 0) + 1 FROM public.group_order_status WHERE group_id = group_uuid),
    total_quantity = (SELECT COALESCE(total_quantity, 0) + quantity FROM public.group_order_status WHERE group_id = group_uuid),
    total_amount = current_total_amount + final_price,  -- Use explicit variable
    updated_at = NOW()
  WHERE group_id = group_uuid;
