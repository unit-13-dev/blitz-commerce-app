-- Debug and fix group order status triggers
-- This migration ensures that group order status is properly updated when payments are made

-- Add debugging to the trigger function
CREATE OR REPLACE FUNCTION trigger_update_group_order_status()
RETURNS TRIGGER AS $$
BEGIN
  -- Log the trigger call for debugging
  RAISE NOTICE 'trigger_update_group_order_status called for group_id: %', NEW.group_id;
...
-- Update the update_group_order_status function to be more robust
...
BEGIN
  ...
  RAISE NOTICE 'Paid participants: %, Total amount: %, Total quantity: %', paid_count, total_amount, total_quantity;
