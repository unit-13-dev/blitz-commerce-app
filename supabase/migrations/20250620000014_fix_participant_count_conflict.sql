-- Fix participant count conflict in create_individual_participant_payment function
-- The issue is that the function manually increments paid_participants, but the trigger also recalculates it
-- This causes the first participant to be counted as 2

-- Update the create_individual_participant_payment function to remove manual participant counting
...
BEGIN
  ...
  -- NOTE: We don't manually update group_order_status here
  -- The trigger will automatically call update_group_order_status() 
  -- which will recalculate paid_participants correctly
