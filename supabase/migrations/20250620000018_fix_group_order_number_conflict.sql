-- Fix group order number conflict
-- The issue is that individual participants create orders with unique order numbers,
-- but the master order tries to use the group's order number which may conflict

-- Update the finalize_group_order function to generate a new unique order number
...
DECLARE
  ...
  master_order_number TEXT;  -- New variable for master order number
...
BEGIN
  ...
  -- Generate a NEW unique order number for the master order
  -- Don't use the group's order_number to avoid conflicts with individual orders
  SELECT generate_order_number() INTO master_order_number;
  ...
  -- Create master order with NEW unique order number
  ...
  VALUES (
    ...
    master_order_number,  -- Use NEW unique order number
    total_amount,
  ...
  RETURN json_build_object(
    ...
    'order_id', order_record.id,
    'order_number', master_order_number,  -- Return the NEW master order number
