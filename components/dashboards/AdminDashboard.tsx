...
const AdminDashboard = () => {
  ...
  // Images from Cloudinary are already full URLs, no need to process
  ...
  // Note: User deletion may require a dedicated admin API route or manual handling in NextAuth
  // For now, we'll throw an error to indicate it is not yet implemented
  const deleteUserMutation = useMutation({
    mutationFn: async (userId: string) => {
      // TODO: Implement user deletion via API route
  ...
  const updateUserRoleMutation = useMutation({
    mutationFn: async ({
      ...
      await apiClient.put(`/profiles/${userId}`, {
        role,
      });
    },
    onSuccess: () => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <div className="text-sm text-gray-600 mt-2">
              ...
              <span className="mr-4">
                Vendors: {users?.filter((u: any) => u.role === "vendor").length || 0}
              </span>
              <span>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="text-sm text-gray-600 mt-2">
              <span className="mr-4">
                Active: {products?.filter((p: any) => p.isActive).length || 0}
              </span>
              <span>
                Inactive: {products?.filter((p: any) => !p.isActive).length || 0}
              </span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            ...
            <p className="text-sm text-gray-600 mt-2">
              {pendingKYCs && pendingKYCs.length > 0
                ? `${pendingKYCs.length} verification${pendingKYCs.length > 1 ? "s" : ""
                } waiting for review`
                  ...
                  <div>
                    ...
                    {kyc.version > 1 && (
                      <p className="text-xs text-blue-600 font-medium">
                        Resubmission #{kyc.version - 1} (Total: {kyc.version})
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="font-medium">
                        Post by {post.user?.fullName || post.user?.email?.split('@')[0] || 'Unknown'}
                      </span>
                    </div>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                      ...
                      <span>Members: {group._count?.members || 0}</span>
                      <span>
                        Created:{" "}
  ...
  const fetchRejectionHistory = async (kyc: any) => {
    try {
      const history = [];
      let currentKycId = kyc.previousKycId;
      ...
      while (currentKycId) {
        try {
          ...
          currentKycId = kycData.previousKycId;
        } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 mt-20 container mx-auto px-2 sm:px-4 py-4 sm:py-8">
          ...
          {selectedKYC && (
            <div className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedKYC.vendor?.fullName || "N/A"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-500">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm">{selectedKYC.businessName}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-500">
                        Business Address
                      </Label>
                      <p className="text-sm">{selectedKYC.businessAddress || selectedKYC.hoAddress}</p>
                    </div>
                    {selectedKYC.gstNumber && (
                      <div>
                        ...
                        <p className="text-sm">{selectedKYC.gstNumber}</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {selectedKYC.gstUrl && (
                    <Card>
                      ...
                      <CardContent className="space-y-3">
                        <DocumentPreview
                          url={selectedKYC.gstUrl}
                          type="gst"
                          onImageClick={() => {
                            setSelectedImageModal(selectedKYC.gstUrl);
                          }}
                        />
                        <Button
                          variant="outline"
                  ...
                  {selectedKYC.panUrl && (
                    <Card>
                      ...
                      <CardContent className="space-y-3">
                        <DocumentPreview
                          url={selectedKYC.panUrl}
                          type="pan"
                          onImageClick={() => {
                            setSelectedImageModal(selectedKYC.panUrl);
                          }}
                        />
                        <Button
                          variant="outline"
                ...
                {!selectedKYC.gstUrl && !selectedKYC.panUrl && (
                  <Card>
                    <CardContent className="text-center py-8">
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <p className="text-sm text-blue-700">
                  Vendor: {selectedKYCHistory.vendor?.fullName || selectedKYCHistory.vendor?.email}
                </p>
                <p className="text-sm text-blue-700">
                  Business: {selectedKYCHistory.businessName}
                </p>
                <p className="text-sm text-blue-700">
                  Submitted: {new Date(selectedKYCHistory.submittedAt).toLocaleString()}
                </p>
                <p className="text-sm text-blue-700">
                  Version: {selectedKYCHistory.version} | Total Submissions: {selectedKYCHistory.submissionCount || selectedKYCHistory.version}
                </p>
              </div>
                            ...
                            <div className="flex-1">
                              ...
                              <div className="space-y-2">
                                <div>
                                  ...
                                  <p className="text-sm">
                                    {new Date(rejection.reviewedAt || rejection.submittedAt).toLocaleString()}
                                  </p>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 text-sm">
                                  <div>
                                    ...
                                    <p>{rejection.businessName}</p>
                                  </div>
                                  <div>
                                    <Label className="text-sm font-medium text-gray-600">
                                      GST Number
                                    </Label>
                                    <p>{rejection.gstNumber || 'N/A'}</p>
                                  </div>
                                  <div>
                                    <Label className="text-sm font-medium text-gray-600">
                                      PAN Number
                                    </Label>
                                    <p>{rejection.panNumber || 'N/A'}</p>
                                  </div>
                                  <div>
                                    <Label className="text-sm font-medium text-gray-600">
                                      Phone Number
                                    </Label>
                                    <p>{rejection.phoneNumber || 'N/A'}</p>
                                  </div>
                                </div>
                              </div>
                            </div>
