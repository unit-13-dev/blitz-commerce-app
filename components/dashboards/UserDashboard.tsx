...
const UserDashboard = () => {
  ...
  const handleBecomeVendor = async () => {
    try {
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.message || 'Failed to become vendor',
        variant: 'destructive',
      });
    }
  };
                ...
                {editingProfile ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{profile?.fullName || 'Not set'}</p>
                    </div>
                    <div>
                      <Label>Role</Label>
                  ...
                  {cartItems?.map((item: any) => (
                    <div key={item.id} className="flex items-center gap-4 p-4 border rounded-lg">
                      <div className="flex-1">
                        ...
                        <p className="text-lg font-semibold">₹{parseFloat(item.product?.price?.toString() || '0').toFixed(2)}</p>
                      </div>
                    </div>
                  ))}
                  ...
                  {orders?.map((order: any) => (
                    <div key={order.id} className="border rounded-lg p-4">
                      <div className="flex justify-between items-start">
                        ...
                        <p className="text-lg font-semibold">₹{parseFloat(order.totalAmount.toString()).toFixed(2)}</p>
                      </div>
                    </div>
                  ))}
