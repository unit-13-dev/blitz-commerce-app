...
const BulkDiscountModal = ({ selectedProductIds, onClose, onSuccess }: BulkDiscountModalProps) => {
    ...
    queryFn: async () => {
      const products = await Promise.all(
        selectedProductIds.map(async (id) => {
          ...
          const tiersRes = await apiClient.get(`/products/${id}/discount-tiers`);
          return {
            id: product.id,
            name: product.name,
            image_url: product.images?.[0]?.imageUrl || product.imageUrl,
            price: parseFloat(product.price.toString()),
            hasTiers: (tiersRes.data?.discountTiers || []).length > 0,
          };
        })
      );
      return products;
    ...
    mutationFn: async () => {
      ...
      for (const productId of selectedProductIds) {
        try {
          ...
          await apiClient.put(`/products/${productId}/discount-tiers`, {
            discountTiers: validTiers.map(tier => ({
              membersRequired: tier.members_required,
  ...
  const handleUndo = async () => {
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['bulk-products-info'] });
