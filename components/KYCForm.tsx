...
const KYCForm: React.FC<KYCFormProps> = ({ onClose, existingData, isInline = false }) => {
  ...
  const checkDuplicates = async () => {
    const { data } = await apiClient.post('/kyc/check-duplicates', {
      gstNumber: form.gst_number,
      panNumber: form.pan_number,
      tanNumber: form.tan_number,
      phoneNumber: form.phone_number,
      existingKycId: existingData?.id,
    });
  ...
  const submitKYCMutation = useMutation({
    mutationFn: async (formData: typeof form) => {
      ...
      const { data } = await apiClient.post('/kyc', {
        ...
        warehouseAddress: formData.warehouse_address,
        phoneNumber: formData.phone_number,
        gstNumber: formData.gst_number,
        gstUrl: gstUrl,
        panNumber: formData.pan_number,
        panUrl: panUrl,
        tanNumber: formData.tan_number,
        turnoverOver5cr: formData.turnover_over_5cr,
      });
    ...
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.response?.data?.message || error.message || 'Failed to submit KYC. Please try again.',
        variant: 'destructive',
      });
      console.error(error);
    },
