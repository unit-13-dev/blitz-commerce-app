...
const GroupOrderCard: React.FC<GroupOrderCardProps> = ({ group }) => {
  ...
  useEffect(() => {
    if (!group.finalizationDeadline || group.adminFinalizedAt) {
      setTimeLeft(null);
      return;
    }
    const deadline = new Date(group.finalizationDeadline).getTime();
    const update = () => {
      const now = Date.now();
  ...
  }, [group.finalizationDeadline, group.adminFinalizedAt]);

  const { data: productDetail } = useQuery({
    queryKey: ['group-product', group.productId],
    queryFn: async () => {
      if (!group.productId) return null;
      const { data } = await apiClient.get(`/products/${group.productId}`);
      return data.product;
    },
    ...
    >
      ...
      >
        ...
        <div className="relative h-80 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900">
          <img
            src={group.product?.imageUrl || "/placeholder.svg"}
        ...
        <CardContent className="p-6 space-y-4">
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-widest text-pink-600 dark:text-pink-300 font-semibold">Access Code</p>
                    <p className="font-mono text-lg font-bold tracking-wider text-gray-900 dark:text-gray-100">
                      {group.accessCode}
                    </p>
                  </div>
                </div>
          ...
          {!group.orderId && group.finalizationDeadline && !group.adminFinalizedAt && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.4 }}
